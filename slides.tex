...
\begin{itemize}
\item use LLL to generate a new basis with a new vector $w$
\item recurse over this new cone set.
\end{itemize}
\end{frame}


\begin{frame}[label=sec-13]{Polar trick}
\begin{itemize}
...
\begin{frame}[label=sec-14]{References}
\begin{itemize}
...
\begin{frame}[label=sec-15]{Thanks!}
Questions?
\end{frame}

\begin{frame}[label=sec-16]{Assuming $\mathcal{F}$ for cones, derive full $\mathcal{F}$: Part 1.2 (Polytopes)}
